...
export default class Episode extends React.Component {
  ...
  async componentDidMount() {
    const id = window.location.pathname.slice(window.location.pathname.length - 7);
  ...
  async loadMoreCharacters() {
    ...
    const episode = await this.api.loadMoreCharactersForEpisode(id, currentCharactersCount);
    this.setState({episode})
    
  }

  render() {
    const { image, episodeId, title, openingCrawl, director, releaseDate, people } = this.state.episode;
    return (
      <div>
        <section>
          ...
          {people &&
            people.edges.map(character => (
              <li key={character.node.id}>
                <img width='55' src={character.node.image} alt={character.node.name} />
                {character.node.name}
              </li>
            ))}
            {
